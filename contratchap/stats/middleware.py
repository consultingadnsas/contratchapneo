...
class VisitorTrackingMiddleware:
    ...
    def __call__(self, request):
        # 1. S'assurer que le visiteur a une clé de session
        if not request.session.session_key:
            request.session.create()

        # 2. Utiliser une clé par date pour réinitialiser quotidiennement et éviter
        #    les collisions avec d'autres logiques qui pourraient utiliser
        #    'has_visited_today'.
        ...
        if not request.session.get(key):
            ...
            # 4. Incrémentation atomique (F()) pour gérer la concurrence
