      ...
      >
        ...
        <slot>
          <option
            ...
            :key="opt.slug ?? opt.code ?? opt.id ?? index"
            :value="opt.slug ?? opt.code ?? opt.name"
          >
            {{ opt.name }}
          </option>
        </slot>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const inputId = computed(() => props.id || `domain-select-${generatedId}`);
