...
export default {
    ...
    setup(props, { emit }) {
        ...
        const profielStore = useProfileStore();
        ...
        const submitForm = async () => {
            ...
            try {
                // 💡 ASTUCE CONTRATCHAP : On formate les données pour correspondre EXACTEMENT 
                // à ce que le CustomedContractSerializer de Django attend.
                const payloadPourDjango = {
                    subject: checkoutform.subject,
                    phone_number: checkoutform.phone_number,
                    email: checkoutform.email,
                    // On fusionne le nom et le type dans la description
                    description: `[Type : ${checkoutform.contract_type}] - [Nom : ${checkoutform.name}]\n\nDétails du besoin :\n${checkoutform.description}`
                };

                // Appel au store en passant le payload formaté
                await profielStore.customContract(payloadPourDjango);
                ...
                // Récupération des erreurs renvoyées par le sérialiseur si nécessaire
